...
public:
   ...
   COrderBlock() : priceTop(0), priceBottom(0), priceMid(0), timeStart(0),
      isBullish(false), isMitigated(false), isActive(true), isVisible(true), mitigatedTop(false), mitigatedMid(false), mitigatedBot(false), tMitTop(0), tMitMid(0), tMitBot(0) {}
   ...
   void UpdateVisuals(int transparency_val, int effectiveDelaySeconds) {
      ...
      // if(isMitigated && visualType == OB_RECTANGLE) dEnd = timeEnd; // REMOVED to allow extension during delay
      ...
      else
      {
         ...
         if(!mitigatedTop)
            ...
            DrawLine(objNameLineTop, priceTop, mitigatedColor, dEnd, STYLE_DOT); // Extend during delay
         else
         ...
         if(!mitigatedMid)
            ...
            DrawLine(objNameLineMid, priceMid, mitigatedColor, dEnd, STYLE_DOT);
         else
         ...
         if(!mitigatedBot)
            ...
            DrawLine(objNameLineBot, priceBottom, mitigatedColor, dEnd, STYLE_DOT);
         else
   ...
   void DrawLine(string name, double price, color clr, datetime tEnd, ENUM_LINE_STYLE style=STYLE_SOLID) {
      if(ObjectFind(0, name) < 0) ObjectCreate(0, name, OBJ_TREND, 0, timeStart, price, tEnd, price);
...
int OnInit() {
   ...
   ListM5.FreeMode(true);
   ListM10.FreeMode(true);
   ListM15.FreeMode(true);
   ListM30.FreeMode(true);
   ListH1.FreeMode(true);
   ListH4.FreeMode(true);
   ListD1.FreeMode(true);
   ListW1.FreeMode(true);
...
void AddOB(CArrayObj &list, bool bull, datetime t, double top, double bot, double open, double close, ENUM_TIMEFRAMES tf, color cbull, color cbear) {
   ...
   string suffix = "" + IntegerToString((long)t) + "" + EnumToString(tf) + (bull?"_B":"_S");
   ...
   for(int i=0; i<list.Total(); i++) {
      ...
      if(ob.visualType == OB_LINES)
      {
         ...
         if(allLinesDone)
         {
            ...
            ob.isActive = false;   // Stop processing
            ob.isVisible = false;  // Ensure cleanup
         }
      }
      else
         // B) OB_RECTANGLE (Standard Box Logic)
      {
         ...
         if(ob.isMitigated)
         {
            datetime timeSince = TimeCurrent() - ob.mitigationTime;
            if(timeSince > effectiveDelaySeconds)
            {
               ob.isVisible = false;
               ob.isActive = false;
            }
